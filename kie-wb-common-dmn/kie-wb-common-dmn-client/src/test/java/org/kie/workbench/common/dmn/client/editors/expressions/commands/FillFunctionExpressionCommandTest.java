...
public class FillFunctionExpressionCommandTest {
    ...
    public void testGetTemporaryExpression() {
        final FunctionDefinition newExpression = mock(FunctionDefinition.class);
        doReturn(newExpression).when(command).getNewExpression();

        final Expression temporaryExpression = command.getTemporaryExpression();

        assertEquals(newExpression, temporaryExpression);
        verify(command).fill(newExpression, expressionProps);
    }
